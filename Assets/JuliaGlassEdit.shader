...
#define ABSORB  float3(.2, 1., .8) //float3(0., 0.5, 1.3)
			...
			static const float PI = 3.1415926535 * 0.5;
			...
			float4 quatMult(float4 q1, float4 q2)
			{
				...
				r.x = q1.x*q2.x -dot(q1.yzw, q2.yzw);
				r.yzw = q1.x*q2.yzw + q2.x*q1.yzw +cross(q1.yzw, q2.yzw);
				//r.yz *= .4*abs(sin(PI*fmod(_Time.y, 2.0)));
				return r;
			}
...
#define ESCAPE_THRESHOLD 1e1
			...
			{
				float4 z = float4(p, 0);
				//float4 zp = float4(pow(fmod(_Time.y, 4), 2), 0., 0, 0);
				float4 zp = float4(1., 0., 0, 0);
				float t = _Time.y*0.25;// *1.6;// 0.1;
				//float4 c = 0.5*float4(cos(t), cos(t*1.1), cos(t*2.3), cos(t*3.1));
				float4 c = .4*float4(2.*cos(t), 1.4*cos(t*1.1), cos(t*2.3), cos(t*3.1));
				//float4 c = .7*fmod(_Time.y*0.125, 1.0);// fmod(_Time.y*0.5, 1.);// *float4(cos(t), cos(t*1.1), cos(t*2.3), cos(t*3.1));
				//float4 c = 0.6*(1.0-1.0*(abs(sin(PI*fmod(_Time.y*.5, 2.0)))));
				c.z = 0.;
				//c.x = 0.;
				iterateIntersect(z, zp, c, maxIterations);
				//iterateIntersect(z, zp, c, 1.0+fmod(_Time.y*0.25-PI, 5.));
				float normZ = length(z);
				float d = .5 * normZ * log(normZ) / length(zp);
				return d;
			}
...
#define EPS 0.02
			...
			float3 Render(in float3 ro, in float3 rd, in float2 uv)
			{
				...
				float transp = 1.;// 1.1;// -.5*sin(_Time.y*0.667);// 1.*fmod(_Time.y*0.667, 1);// -1.5*fmod(_Time.y, 1.0);
				//float3 absorb = float3(uv.y, 0.5 - 0.25*sin(_Time.y), 0.25*sin(_Time.y) + uv.x);// *fmod(_Time.y, 1.5);// ABSORB * sin(_Time.y);
				float3 absorb = float3(0.5 + 0.5*sin(_Time.y), uv.y, uv.x)*0.6*sin(_Time.y*2.);// fmod(_Time.y*0.667, 1);
            ...
            fixed4 frag (v2f i) : SV_Target
            {
				...
				for (int n = 0; n < 1; n++)
				{
					...
					float theta =  _MapY+0.05*sin(_Time.y*0.33);// radians(360.)*
					float phi = 2.05+ 0.05*sin(_Time.y*0.25);// radians
					float3 ro = /*2.5*/2.6*float3(sin(phi)*cos(theta), cos(phi), sin(phi)*sin(theta));
					//float3 ro = /*2.5*/2.6*float3(0.01, 1., 0);
